...
public class RegistrationService {
    ...
    public Registration searchByRegistrationNumber(String keyword) {
        return registrationRepository.findByRegistrationNumberContainingIgnoreCase(keyword);
    }
    
    public String searchByRegistrationNumberOwner(String keyword) {
    	Registration register = registrationRepository.findByRegistrationNumberContainingIgnoreCase(keyword);
    	return register.getOwner().getName();
    }
	...
	public Registration updateOwnerName(String registrationNumber, String newName) {
		// TODO Auto-generated method stub
		boolean owner = ownerRepository.existsByName(newName);
		if(owner) {
			Owner ExistedOwner = ownerRepository.findByName(newName);
			Registration updated = registrationRepository.findByRegistrationNumberContainingIgnoreCase(registrationNumber);
			updated.setOwner(ExistedOwner);
			return registrationRepository.save(updated);
		}
		else {
			throw new RuntimeException("Owner Not exits");
		}
	}
